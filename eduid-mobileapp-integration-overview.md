...
Recent implementation of OAuth2 build on JSON Web-Tokens (JWT). JWT provides a more flexible and more secure authentication and authorization approach than the original (plain) [Bearer scheme for OAuth2](https://tools.ietf.org/html/rfc6750). Different to OAuth1 MAC authorization, JWT is fully compatible to the Bearer scheme. Appropriate handling of JWT bearer authorization is subject to [RFC 7800](https://tools.ietf.org/html/rfc7800).

#### Implementation Issues

The OAuth2 specification defines a framework for authentication and authorization. It does not specify identity provisioning. Exchaning identity related information between an authorzing party and an resource service. Typically the authorizing party is identical with an identity provider (IDP). Different to SAML, an IDP and the Authorizing party MAY refer to the same service. Authorizing tokens may encapsulate identity information in the authorization code for a client in the form of a secured or encrypted JWT. Such tokens allow resource services to validate the authenticity of an authrization request without exchanging information with the authrizing service.

The organisation of provided identity information is partially covered by the OpenID Connect specification [OPENID CONNECT 1.0]. Actual implementations of OAuth2/OpenID Connect Identity Providers (IDP) may include additional/federation-specific identity information.

The EduID Mobile App is agnostic to the IDP related implementation as it uses and passes JWT tokens without manipulating identity related information.
